...
import { Folder, Star, ExternalLink, Calendar, User, Tag, Building, Clock, Search, Filter } from "lucide-react";
...
const ProjectsPage = () => {
  ...
  const categories = ["All", "E-Learning", "E-Commerce", "Management System", "Academic Project", "Web Application", "Enterprise"];
  ...
  useEffect(() => {
    ...
    if (metaDescription) {
      metaDescription.setAttribute("content", "Explore our portfolio of innovative software solutions including ThinkForge e-learning platform, ViorMart e-commerce system, and enterprise management solutions.");
    }
        ...
        <div className="container relative">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-20">
            {filteredProjects.map((project) => (
              <div key={project.id} className="bg-n-7 rounded-xl p-8 border border-n-6 hover:border-color-1 transition-all duration-300 group">
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-center gap-3">
                    <Folder className="w-10 h-10 text-color-1" />
                    <div>
                      <h3 className="text-xl font-bold text-n-1 group-hover:text-color-1 transition-colors">{project.title}</h3>
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                    ...
                    project.status === 'In Production' ? 'bg-blue-500/20 text-blue-400' :
                ...
                <p className="text-n-3 mb-4 leading-relaxed">{project.description}</p>
                
                <div className="flex flex-wrap gap-2 mb-6">
                  <div className="flex items-center gap-2 px-3 py-1 bg-color-1/10 rounded-full">
                    <Tag className="w-4 h-4 text-color-1" />
                    <span className="text-color-1 text-sm font-medium">{project.category}</span>
                  </div>
                ...
                <div className="mb-6">
                  <h4 className="text-n-1 font-semibold mb-3">Key Technologies:</h4>
                  <div className="grid grid-cols-1 gap-2">
                    ...
                    {project.technologies.length > 4 && (
                      <span className="text-color-1 text-sm ml-5">+{project.technologies.length - 4} more features</span>
                    )}
                  </div>
                </div>
          ...
          {filteredProjects.length === 0 && (
            <div className="text-center py-12 mb-20">
              ...
              <h3 className="text-xl font-semibold text-n-1 mb-2">No projects found</h3>
              <p className="text-n-3">Try adjusting your search terms, category, or status filter.</p>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              >
                ...
                <p className="text-color-1 text-xs mt-2 font-medium">
                  {projects.filter(p => p.category.includes("Academic")).length} projects
                </p>
              </div>
              ...
              >
                ...
                <p className="text-color-1 text-xs mt-2 font-medium">
                  {projects.filter(p => p.category.includes("E-Commerce") || p.category.includes("Web Application")).length} projects
                </p>
              </div>
              ...
              >
                ...
                <p className="text-color-1 text-xs mt-2 font-medium">
                  {projects.filter(p => p.category.includes("Enterprise") || p.category.includes("Management")).length} projects
                </p>
              </div>
            </div>
          </div>
